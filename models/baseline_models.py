...
class SimpleAutoencoder(nn.Module):
    def __init__(self, config):
        '''
        Simple Autoencoder, input is (batch_size, seq_length, feat_dim)
        '''
        super(SimpleAutoencoder, self).__init__()
        self.fc = nn.Linear(in_features=config.feat_dim*config.ts_length, out_features=config.emb_size)

    def forward(self, x):
        x = x.view(x.shape[0], -1) # (batch_size, seq_length * feat_dim)
        x = self.fc(x)
        return x
...
class DeepAutoencoder(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = nn.Linear(in_features=config.feat_dim*config.ts_length, out_features=240)
...
class LSTMencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        emb = self.fc(emb)
        emb = emb.reshape(emb.shape[0], emb.shape[1] * emb.shape[2])
        return emb     
