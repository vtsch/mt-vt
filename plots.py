...
def plot_representations_of_cluster(config: Bunch, data: np.ndarray, labels: np.ndarray, cluster_id: int, label_color_map: list, experiment) -> None:
    ...
    print("Cluster %d has %d datapoints predicted" %
          (cluster_id, datapoints.shape[0]))

    title1 = "datapoints_cluster_%d_%s_%s" % (
        cluster_id, config.experiment_name, config.pos_enc)
    ...
    title2 = "boxplot_cluster_%d_%s_%s" % (
        cluster_id, config.experiment_name, config.pos_enc)
    fig2 = plt.figure(title2)
    # create boxplot of datapoints of cluster_id for each timestep
    plt.boxplot(datapoints, widths=0.6, patch_artist=True,
                boxprops=dict(facecolor=label_color_map[cluster_id]))
    plt.ylim(0, 2)
    plt.title(title2)
    plt.legend(labels=[cluster_id], loc='upper left', title="Clusters")
...
def plot_all_representations(config: Bunch, data: np.ndarray, labels: np.ndarray, experiment) -> None:
    ...
    for i in range(len(data)):
        ...
        plt.ylim(0, 2)
    plt.title(title)
    plt.legend(['%d' % i for i in range(np.unique(labels).shape[0])],
    ...
    for i in range(np.unique(labels).shape[0]):
        plot_representations_of_cluster(
            config, data, labels, i, label_color_map, experiment)
...
def plot_centroids(config: Bunch, centroids: np.ndarray, experiment) -> None:
    ...
    Args:
        ...
        experiment: comet.ml experiment to log the figure
    '''
    title = "%s kmeans centroids %s" % (
        config.kmeans_metric, config.experiment_name)
    for i in range(config.n_clusters):
        plt.plot(centroids[i])
    plt.title(title)
    plt.ylim(0, 2)
    plt.legend(['%d' % i for i in range(config.n_clusters)],
               loc='upper left', title="Clusters")
...
def plot_umap(config: Bunch, umap_embedding: np.ndarray, y_pred: np.ndarray, y_real: np.ndarray, experiment) -> None:
    ...
    scatter1 = ax1.scatter(
        umap_embedding[:, 0], umap_embedding[:, 1], s=2, c=y_pred, cmap=cmap, norm=norm)
    ax1.set_title('Real')
    ax1.legend(*scatter1.legend_elements(), loc="upper left", title="Clusters")

    scatter2 = ax2.scatter(
        umap_embedding[:, 0], umap_embedding[:, 1], s=2, c=y_real, cmap=cmap, norm=norm)
    ax2.set_title('Predicted')
    ax2.legend(*scatter2.legend_elements(), loc="upper left", title="Clusters")

    experiment.log_figure(figure=plt, figure_name="umap_%s" %
                          config.experiment_name)
    plt.savefig(os.path.join(config.model_save_path,
                "umap_%s.png" % config.experiment_name))
...
def run_umap(config: Bunch, output: np.ndarray, true_labels: np.ndarray, kmeans_labels: np.ndarray, experiment) -> None:
    ...
    Args:
        ...
        experiment: comet.ml experiment to log the figure
    '''
    umap_embedding = umap.UMAP(n_neighbors=15, min_dist=0.1,
                               metric='correlation', n_components=2).fit_transform(output)
